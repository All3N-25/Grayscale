...
int main(){
    ...
    const size_t image_size = row_stride * std::abs(info_header.height); //Dahil hindi supported vector gagawin char

    // FIX: use raw pointer instead of vector for OpenACC compatibility
    char* image_data = new char[image_size];

    myReadFile.read(image_data, image_size);
    myReadFile.close();
    ...
    */
    ...
    #pragma acc data copy(image_data[0:image_size])
    {
        auto start2 = std::chrono::high_resolution_clock::now();
        #pragma acc parallel loop collapse(2) //combine multiple nested loops into a single loop
        for(int y = 0; y < std::abs(info_header.height); y++){     //vertical | column
        ...
        std::chrono::duration<double, std::milli> elapsed = end - start2;
        std::cout << "Grayscale conversion took " << elapsed.count() << " ms without copying data to the GPU" << std::endl;
    }

    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> elapsed = end - start1;
    std::cout << "Grayscale conversion took " << elapsed.count() << " ms with copying data to the GPU" << std::endl;
    myWriteFile.write(image_data, image_size);
    myWriteFile.close();

    delete[] image_data; // free memory
